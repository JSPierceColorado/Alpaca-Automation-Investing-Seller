...
# from alpaca.common.types import BaseURL   # ← removed, not needed
...
ALPACA_BASE_URL = os.getenv("ALPACA_BASE_URL", "https://paper-api.alpaca.markets")  # optional hint
...
def make_alpaca_clients() -> Tuple[TradingClient, StockHistoricalDataClient]:
    if not (ALPACA_API_KEY and ALPACA_API_SECRET):
        raise RuntimeError("ALPACA_API_KEY or ALPACA_API_SECRET missing.")

    # Determine paper vs live from ALPACA_BASE_URL (or let users override by setting a PAPER flag if you prefer)
    is_paper = "paper" in (ALPACA_BASE_URL or "").lower()

    trading = TradingClient(
        api_key=ALPACA_API_KEY,
        secret_key=ALPACA_API_SECRET,
        paper=is_paper,            # ← rely on this; no base_url arg
        # base_url=BaseURL(...)    # ← removed
    )
